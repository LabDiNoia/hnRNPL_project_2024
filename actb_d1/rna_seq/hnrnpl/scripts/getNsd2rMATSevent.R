...
jc_file<-paste0(rmats_dir,"SE.MATS.JC.txt")

library(data.table)
jc_dt<-fread(jc_file)
jc_dt<-jc_dt[FDR<0.1]
jc_sirt1_dt<-jc_dt[grep("Nsd2",geneSymbol,ignore.case=T)]

if (nrow(jc_sirt1_dt)>0) {
	write("export NSD2_RMATS_EVENT=Yes", file=pipeline_vars_file, append=T)
	write.table(jc_sirt1_dt, paste0(rmats_dir,"Nsd2_rMATS_SE_event.txt"),
	...
	write("export NSD2_RMATS_EVENT=No", file=pipeline_vars_file, append=T)
}
