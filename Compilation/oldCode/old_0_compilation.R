...
extractAlnDetails(alnInfo_matList,nmatch)
extractAlnDetails(alnInfo_matList,nmismatch)
...
Reduce(f=function(x,y){x&y}, x=sapply(alnPID,function(x)x>70,simplify=F))

Reduce(f=function(x,y){x+y}, x=extractAlnDetails(alnInfo_matList,score))

# the order is important!! mouse_gr then human_gr for it to be consistent with other stuff
outer(width(mouse_gr),width(human_gr),FUN=pmax)
