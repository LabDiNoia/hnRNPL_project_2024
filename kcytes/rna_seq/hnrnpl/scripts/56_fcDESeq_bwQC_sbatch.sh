...
then
	...
	REFR_MERGED_FWD_BW="${BW_DIR}/${REFR_SAMPLES[0]}_fwd.bw"
	REFR_MERGED_REV_BW="${BW_DIR}/${REFR_SAMPLES[0]}_rev.bw"
else
	for i in ${!REFR_SAMPLES[@]}; do 
		REFR_FWD_BWS[$i]="${BW_DIR}/${REFR_SAMPLES[$i]}_fwd.bw"
		REFR_REV_BWS[$i]="${BW_DIR}/${REFR_SAMPLES[$i]}_rev.bw"
	done

	echo $(date -Iseconds)" SPG_RNAseq_pipeline: 6. Merging ${#REFR_SAMPLES[@]} WT bigwig files.."
	REFR_MERGED_FWD_BW="${BW_DIR}/${REFR_GROUP_NAME}_merged_fwd.bw"
	REFR_MERGED_REV_BW="${BW_DIR}/${REFR_GROUP_NAME}_merged_rev.bw"
	wiggletools mean ${REFR_FWD_BWS[@]} | wigToBigWig stdin $CHROM_SIZE_FILE ${REFR_MERGED_FWD_BW}
	wiggletools mean ${REFR_REV_BWS[@]} | wigToBigWig stdin $CHROM_SIZE_FILE ${REFR_MERGED_REV_BW}

	REFR_FWD_ABS_BW="${SLRM_LOCAL_TMP_DIR}/${REFR_GROUP_NAME}_fwd_unstranded.bw"
	REFR_REV_ABS_BW="${SLRM_LOCAL_TMP_DIR}/${REFR_GROUP_NAME}_rev_unstranded.bw"
	REFR_NOSTRAND_BW="${BW_DIR}/${REFR_GROUP_NAME}_merged_unstranded.bw"
	wiggletools abs ${REFR_MERGED_FWD_BW} | wigToBigWig stdin $CHROM_SIZE_FILE ${REFR_FWD_ABS_BW}
	wiggletools abs ${REFR_MERGED_REV_BW} | wigToBigWig stdin $CHROM_SIZE_FILE ${REFR_REV_ABS_BW}
	wiggletools sum ${REFR_MERGED_FWD_BW} ${REFR_MERGED_REV_BW} | \
	wigToBigWig stdin $CHROM_SIZE_FILE ${REFR_NOSTRAND_BW}
	rm $REFR_FWD_ABS_BW $REFR_REV_ABS_BW
fi
...
then
	...
	TEST_MERGED_FWD_BW="${BW_DIR}/${TEST_SAMPLES[0]}_fwd.bw"
	TEST_MERGED_REV_BW="${BW_DIR}/${TEST_SAMPLES[0]}_rev.bw"
else

	for i in ${!TEST_SAMPLES[@]}; do 
		TEST_FWD_BWS[$i]="${BW_DIR}/${TEST_SAMPLES[$i]}_fwd.bw"
		TEST_REV_BWS[$i]="${BW_DIR}/${TEST_SAMPLES[$i]}_rev.bw"
	done

	echo $(date -Iseconds)" SPG_RNAseq_pipeline: 6. Merging ${#TEST_SAMPLES[@]} KO bigwig files.."
	TEST_MERGED_FWD_BW="${BW_DIR}/${TEST_GROUP_NAME}_merged_fwd.bw"
	TEST_MERGED_REV_BW="${BW_DIR}/${TEST_GROUP_NAME}_merged_rev.bw"
	wiggletools mean ${TEST_FWD_BWS[@]} | wigToBigWig stdin $CHROM_SIZE_FILE ${TEST_MERGED_FWD_BW}
	wiggletools mean ${TEST_REV_BWS[@]} | wigToBigWig stdin $CHROM_SIZE_FILE ${TEST_MERGED_REV_BW}
	
	TEST_FWD_ABS_BW="${SLRM_LOCAL_TMP_DIR}/${TEST_GROUP_NAME}_fwd_unstranded.bw"
	TEST_REV_ABS_BW="${SLRM_LOCAL_TMP_DIR}/${TEST_GROUP_NAME}_rev_unstranded.bw"
	TEST_NOSTRAND_BW="${BW_DIR}/${TEST_GROUP_NAME}_merged_unstranded.bw"
	wiggletools abs ${TEST_MERGED_FWD_BW} | wigToBigWig stdin $CHROM_SIZE_FILE ${TEST_FWD_ABS_BW}
	wiggletools abs ${TEST_MERGED_REV_BW} | wigToBigWig stdin $CHROM_SIZE_FILE ${TEST_REV_ABS_BW}
	wiggletools sum ${TEST_MERGED_FWD_BW} ${TEST_MERGED_REV_BW} | \
	wigToBigWig stdin $CHROM_SIZE_FILE ${TEST_NOSTRAND_BW}
	rm $TEST_FWD_ABS_BW $TEST_REV_ABS_BW
fi
...
then
	...
	#do diff
	bigwigCompare -b1 ${TEST_NOSTRAND_BW} -b2 ${REFR_NOSTRAND_BW} \
